...
See https://github.com/amankapoor/goth/examples/main.go to see this in action.
*/
package gothic
...
See https://github.com/amankapoor/goth/examples/main.go to see this in action.
*/
func BeginAuthHandler(res http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	url, err := GetAuthURL(res, req, ps)
	if err != nil {
...
*/
func GetAuthURL(res http.ResponseWriter, req *http.Request, ps httprouter.Params) (string, error) {
	if !keySet && defaultStore == Store {
		fmt.Println("goth/gothic: no SESSION_SECRET environment variable is set. The default cookie store is not available and any calls will fail. Ignore this warning if you are using a different store.")
	}

	providerName, err := GetProviderName(req, ps)
	if err != nil {
		return "", err
	}
...
See https://github.com/amankapoor/goth/examples/main.go to see this in action.
*/
var CompleteUserAuth = func(res http.ResponseWriter, req *http.Request, ps httprouter.Params) (goth.User, error) {
	if !keySet && defaultStore == Store {
		fmt.Println("goth/gothic: no SESSION_SECRET environment variable is set. The default cookie store is not available and any calls will fail. Ignore this warning if you are using a different store.")
	}
...
func Logout(res http.ResponseWriter, req *http.Request, ps httprouter.Params) error {
	providerName, err := GetProviderName(req, ps)
	if err != nil {
		return err
	}
...
func getProviderName(req *http.Request, ps httprouter.Params) (string, error) {
	...
	// try to get it from the url param "provider" (httprouter)
	if p := ps.ByName("provider"); p != "" {
		return p, nil
	}
